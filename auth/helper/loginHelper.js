...
export const authManager = (callback = () => {}) => {
    const setAuthorization = async ({ isFinished = false } = {}) => {
        if (isFinished) {
            ...
            const token = await _UserClientLibs('getIdTokenCookie')();
            callback({ accessToken, token });
        }
    };

    window.LN.observable.subscribe('statusRotationAuth', setAuthorization);
};

const initializeAuth = async callback => {
    try {
        const response = await _UserClientLibs(
            'GetAccessTokenValidatedAsync'
        )();

        window?.LN?.observable?.publish('statusRotationAuth', {
            isFinished: true
        });
        ...
        console.error('Error occurred while executing token rotation');
    }
};
