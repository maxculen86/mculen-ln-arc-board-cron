...
const preloadFirstImage = items => {
    const firstImgSrc = getArticleImageUrl(items[0]);
    if (!firstImgSrc) return;
    try {
        const { origin } = new URL(firstImgSrc);
        preconnect(origin);
    } catch (_) {
        /* URL inválida */
    }
    preload(firstImgSrc, { as: 'image', fetchPriority: 'high' });
};
    ...
    useEffect(() => {
        ...
        const fetchSearchResults = async (
            ...
            try {
                ...
                const apiUrl = `${URL_BASE}&query=${encodeURIComponent(query)}&sort=${sortParam}&batchsize=24&showfaceted=true${appliedFilters}${dateRangeParam}&endindex=${numberPage}`;
                ...
                if (numberPage === 0) {
                    if (topics.length > 0) preloadFirstImage(topics);
                    if (items.length > 0) preloadFirstImage(items);
                }
