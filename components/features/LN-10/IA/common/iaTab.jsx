import React from 'react';
import { Text } from '@ln/common-ui-text';
import { Icon } from '@ln/common-ui-icon';
import PropTypes from 'prop-types';

export function IaTab({ id = '', title = '' }) {
    if (!id || !title) return null;
    // TODO: utilizar SVGS con <IconSprite> y en caso que fallen colores, consumir como asset estatico
    const glossary = id === 'glossary' && (
        <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <rect
                opacity="0.5"
                x="4"
                y="4.11182"
                width="15"
                height="18"
                rx="3"
                fill="url(#paint0_linear_9717_190)"
            />
            <path
                d="M18.713 8.96752L19.0841 8.09748C19.4143 7.32326 20.0087 6.70693 20.7502 6.37003L21.7716 5.90592C22.0963 5.75838 22.0963 5.27479 21.7716 5.12724L20.782 4.67762C20.0215 4.33205 19.4165 3.69291 19.0919 2.89226L18.716 1.96508C18.5765 1.62101 18.1121 1.62101 17.9726 1.96508L17.5967 2.89225C17.2721 3.69291 16.667 4.33205 15.9065 4.67762L14.917 5.12724C14.5923 5.27479 14.5923 5.75838 14.917 5.90592L15.9384 6.37003C16.6799 6.70693 17.2743 7.32326 17.6045 8.09748L17.9756 8.96752C18.1182 9.30191 18.5703 9.30191 18.713 8.96752Z"
                fill="#27D2BE"
            />
            <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M5.74484 6.72227C5.74484 6.22111 6.13544 5.81485 6.61726 5.81485H12.6249C13.1068 5.81485 13.5009 5.40858 13.5009 4.90742C13.5009 4.40627 13.1068 4 12.6249 4H6.61726C5.17179 4 4 5.2188 4 6.72227V19.0632C4 19.0855 4.00077 19.1075 4.00228 19.1294C4.00077 19.1674 4 19.2056 4 19.2441C4 20.7475 5.17179 21.9663 6.61726 21.9663H16.9118C17.7791 21.9663 18.4822 21.2351 18.4822 20.333V18.3373C18.964 18.3373 19.3546 17.931 19.3546 17.4299V11.1686C19.3546 10.6675 18.964 10.2612 18.4822 10.2612C18.0003 10.2612 17.6098 10.6675 17.6098 11.1686V16.5218H6.61726C6.59818 16.5218 6.57914 16.522 6.56016 16.5224H6.18105C6.02214 16.5224 5.87316 16.5666 5.74484 16.6438V6.72227ZM6.58431 18.3373H16.7373V20.1515H6.61726C6.13544 20.1515 5.74484 19.7452 5.74484 19.2441C5.74484 18.7544 6.11776 18.3553 6.58431 18.3373Z"
                fill="#4D4D4D"
            />
            <defs>
                <linearGradient
                    id="paint0_linear_9717_190"
                    x1="11.5"
                    y1="4.11182"
                    x2="11.5"
                    y2="29.1392"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="white" />
                    <stop offset="1" stopColor="#27D2BE" />
                </linearGradient>
            </defs>
        </svg>
    );

    const summary = id === 'summary' && (
        <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <rect
                opacity="0.5"
                x="4"
                y="4.01456"
                width="15.1825"
                height="16.9708"
                rx="1"
                fill="url(#paint0_linear_9754_255)"
            />
            <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M5.70817 5.57191H12.438C12.9111 5.57191 13.2945 5.17957 13.2945 4.69559C13.2945 4.21161 12.9111 3.81927 12.438 3.81927H5.09636C4.48816 3.81927 3.99512 4.32371 3.99512 4.94597V19.9685C3.99512 20.5908 4.48816 21.0952 5.09636 21.0952H18.0666C18.6748 21.0952 19.1678 20.5908 19.1678 19.9685V10.7046C19.1678 10.2206 18.7844 9.8283 18.3113 9.8283C17.8383 9.8283 17.4548 10.2206 17.4548 10.7046V19.3426H5.70817V5.57191Z"
                fill="#4D4D4D"
            />
            <path
                d="M7.32117 11.6909H15.9586V13.4184H7.32117V11.6909Z"
                fill="#4D4D4D"
            />
            <path
                d="M11.6399 9.11311L9.91241 7.3722L9.04866 8.44518L11.6399 10.8272L14.2311 8.44518L13.3674 7.3722L11.6399 9.11311Z"
                fill="#4D4D4D"
            />
            <path
                d="M11.6399 15.9963L13.3674 17.7372L14.2311 16.6642L11.6399 14.2822L9.04866 16.6642L9.91241 17.7372L11.6399 15.9963Z"
                fill="#4D4D4D"
            />
            <path
                d="M18.5576 8.32585L18.9287 7.45581C19.2589 6.6816 19.8533 6.06527 20.5948 5.72837L21.6162 5.26426C21.9409 5.11671 21.9409 4.63312 21.6162 4.48557L20.6266 4.03595C19.8661 3.69039 19.2611 3.05125 18.9365 2.2506L18.5606 1.32342C18.4211 0.97935 17.9567 0.979351 17.8172 1.32342L17.4413 2.25059C17.1167 3.05125 16.5116 3.69039 15.7511 4.03595L14.7616 4.48557C14.4369 4.63312 14.4369 5.11671 14.7616 5.26426L15.783 5.72837C16.5245 6.06527 17.1189 6.6816 17.4491 7.45581L17.8202 8.32585C17.9628 8.66025 18.4149 8.66025 18.5576 8.32585Z"
                fill="#27D2BE"
            />
            <path
                d="M18.5576 8.32585L18.9287 7.45581C19.2589 6.6816 19.8533 6.06527 20.5948 5.72837L21.6162 5.26426C21.9409 5.11671 21.9409 4.63312 21.6162 4.48557L20.6266 4.03595C19.8661 3.69039 19.2611 3.05125 18.9365 2.2506L18.5606 1.32342C18.4211 0.97935 17.9567 0.979351 17.8172 1.32342L17.4413 2.25059C17.1167 3.05125 16.5116 3.69039 15.7511 4.03595L14.7616 4.48557C14.4369 4.63312 14.4369 5.11671 14.7616 5.26426L15.783 5.72837C16.5245 6.06527 17.1189 6.6816 17.4491 7.45581L17.8202 8.32585C17.9628 8.66025 18.4149 8.66025 18.5576 8.32585Z"
                fill="#27D2BE"
            />
            <defs>
                <linearGradient
                    id="paint0_linear_9754_255"
                    x1="11.5912"
                    y1="4.01456"
                    x2="12"
                    y2="31.5"
                    gradientUnits="userSpaceOnUse"
                >
                    <stop stopColor="white" />
                    <stop offset="1" stopColor="#27D2BE" />
                </linearGradient>
            </defs>
        </svg>
    );

    return (
        <>
            <Icon size={24}>
                {glossary}
                {summary}
            </Icon>
            <Text
                as="h2"
                className="--font-bold text-20 as-flex-end"
                text={title}
            />
        </>
    );
}
IaTab.propTypes = {
    id: PropTypes.string,
    title: PropTypes.string
};
IaTab.defaultProps = {
    id: '',
    title: ''
};

export default IaTab;
