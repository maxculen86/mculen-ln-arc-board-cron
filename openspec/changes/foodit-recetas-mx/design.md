...
> ⚠️ **ABIERTO — pendiente Arquitectura:** el mecanismo de CONSUMO de libs desde bundles Fusion NO es el path alias de `tsconfig.base.json` (Fusion no lo lee). Ver el documento de decisión completo con hallazgos de la doc oficial de Arc XP y opciones (copy / `file:`-symlink / publish versionado): [`docs/migrate-mx/libs-strategy/fusion-libs-resolution.md`](../../../docs/migrate-mx/libs-strategy/fusion-libs-resolution.md). La sub-decisión de abajo (generator, estructura) sigue válida; lo que queda por decidir es copy-vs-lib-consumible y publishable-vs-no.
