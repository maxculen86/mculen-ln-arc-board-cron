...
const _setCookie = (nameCookie, valueCookie, timeExpiration) => {
    ...
    const exp = new Date(new Date().getTime() + seconds);

    if (typeof document !== 'undefined') {
        document.cookie = `${nameCookie}=${valueCookie}; expires=${exp.toUTCString()}; domain=.lanacion.com.ar; path=/`;
    }
    return !!(
        document &&
        document.cookie &&
    ...
    return new Promise((resolve, reject) => {
        if ('serviceWorker' in navigator) {
            navigator.serviceWorker
                ...
                .catch(err => {
                    console.log('SW: unreg ERROR', arguments);
                    reject(err);
                });
        } else {
...
const initialize = () => {
    ...
    if (notifButtonNo && notifButtonYes) {
        notifButtonNo.addEventListener('click', e => {
            ...
            _setCookie('ln-notification', 'false', 43200);
        ...
        notifButtonYes.addEventListener('click', e => {
            ...
            _setCookie('ln-notification', 'true', 43200);
            showNoShowModal('#notificacion-modal', 'none');
...
const start = () => {
    ...
    const deployment = document
        .getElementById('pwaScript')
        .getAttribute('deployment');
    console.log(
        '🚀 ~ file: register.js ~ line 555 ~ start ~ deployment',
        deployment
    );
    ...
    if (!isSafari && !iOS) {
        if (verify()) {
            register(deployment)
                .then(swRegistration => {
                    console.log('SW: start', swRegistration);
                })
                .catch(err => {
            ...
            unregister()
                ...
                .catch(err => console.error('SW: unreg', err));
        }
    }
    ...
    return new Promise((resolve, reject) => {
        /*
        jquery(function() {
            if (window.banner && window.banner.sites) {
                let ad = new EplAd(window.banner.sites, window.keywords);
                ad.initSpaces();
            }
            if (!window.render) {
                resolve();
                ...
                Spinner.show();
                page.loadData(window.location.href)
                    .then(rs => {
                        Spinner.hide();
                        resolve(rs);
                    })
