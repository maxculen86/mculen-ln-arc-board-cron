...
export const extractTime = (isoString = '') => isoString.slice(11, 16);

const reorderLocations = (endpointData, children = [], serviceItem = '') => {
    if (!serviceItem) return endpointData;

    return children.reduce((acc, loc = {}) => {
        const province = endpointData.find(
            e => removeAccents(e.location_name) === removeAccents(loc.name)
        );

        return province ? [...acc, province] : acc;
    }, []);
};

export const validateMinTemperature = (tempMin, currTemp) =>
    tempMin > currTemp ? Math.floor(currTemp) : tempMin;

export const validateMaxTemperature = (tempMax, currTemp) =>
    tempMax < currTemp ? Math.ceil(currTemp) : tempMax;
