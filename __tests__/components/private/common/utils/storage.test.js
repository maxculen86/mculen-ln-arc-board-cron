...
describe('Common - Utils - Storage', () => {
    ...
    it("counterNota deberia llamar a localStorage", () => {
      ...
      it("filterNotesWithinDays deberia filtrar nota anterior a 28 dias", () => {
        const notasCounter = [{"notaId":"6Q4WDU7YVJBEZEOLSQEIK3YCYI","fecha":"2020-6-5"},{"notaId":"AVYWDWDAVVESZGD7HXMW46GTYA","fecha":"2020-8-5"},{"notaId":"SONLF2WL3JAPRINOFCPEDZE5Y4","fecha":"2020-8-6"},{"notaId":"O6VUCJ32TRG37LUFMGRFVBETBM","fecha":"2020-8-6"}];
        const newNotasCounter = filterNotesWithinDays(notasCounter);

        expect(newNotasCounter.length).toBe(3);
        expect(window.localStorage.getItem).toHaveBeenCalledTimes(1);
