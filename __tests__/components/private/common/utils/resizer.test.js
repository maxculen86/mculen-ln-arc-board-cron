...
describe('LN - Common - Utils - Imagen - Resizer', () => {
    ...
    it('Test de resizeo horizontal', () => {
        ...
        expect(newNota.resized_urls.big.resizedUrl).toBe(
        ...
        expect(newNota.resized_urls.medium.resizedUrl).toBe(
        ...
        Object.keys(presets.big).forEach(p => {
            expect(newNota.resized_urls.big.option[p]).toBe(presets.big[p]);
        });
    });
    ...
    it('Test de resizeo en nota ANS', () => {
        ...
        expect(newNota.promo_items.basic.resized_urls.big.resizedUrl).toBe(
            'http://demo-prod.origin.arcpublishing.com/resizer/k_-nxquhw4MOtCGTufFgn1lPu1I=/200x0/arc-anglerfish-arc2-sandbox-sandbox-lanacionar.s3.amazonaws.com/public/VASAYYYBLVFIJCFDSH22JS5X2Q.jpg'
        );
        Object.keys(presets.big).forEach(p => {
            expect(newNota.promo_items.basic.resized_urls.big.option[p]).toBe(
                presets.big[p]
            );
        });

        expect(newNota.content_elements[3].resized_urls.big.resizedUrl).toBe(
        ...
        Object.keys(presets.big).forEach(p => {
            expect(newNota.content_elements[3].resized_urls.big.option[p]).toBe(
                presets.big[p]
            );
        });

        expect(newNota.credits.by[0].image.resized_urls.big.resizedUrl).toBe(
        ...
        Object.keys(presets.big).forEach(p => {
            expect(newNota.credits.by[0].image.resized_urls.big.option[p]).toBe(
                presets.big[p]
            );
        });
    });
